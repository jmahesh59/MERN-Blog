      ...
      {currentUser.isAdmin && userPosts.length>0 ?(<>
        <Table hoverable className='shadow-md'>
          <Table.Head>
            ...
            <Table.HeadCell><spam>Edit</spam> </Table.HeadCell>
          </Table.Head>
          {userPosts.map((post)=>(
              ...
              <Table.Row className='bg-white dark:border-gray-700 dark:bg-gray-800'>
                ...
                <Table.Cell>
                  <Link to={`/post/${post.slug}`}>
                    <img src={post.image} alt={post.tittle} className='w-20 h-10 object-cover bg-gray-500'/>
                  </Link> 
                </Table.Cell>
                <Table.Cell>
                ...
                <Table.Cell className='text-teal-400'><Link to={`/update-post/${post._id}`}><span>Edit</span></Link></Table.Cell>
              </Table.Row>
            </Table.Body>
          ))}
        </Table>
        {
          showMore && (
            <button className='w-full text-teal-500 self-center p-7' onClick={handleShowMore}>Show more</button>
          )
        }
      </>):(
