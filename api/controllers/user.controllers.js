...
export const updateUser = async (req, res, next) => {
  ...
  if (req.body.username) {
    if (req.body.username.length < 7 || req.body.username.length > 20) {
      return next(
        errorHander(400, 'Username must be between 7 and 20 characters')
      );
    }
    if (req.body.username.includes(' ')) {
      return next(errorHander(400, 'Username cannot contain spaces'));
    ...
    if (req.body.username !== req.body.username.toLowerCase()) {
      return next(errorHander(400, 'Username must be lowercase'));
    }
    if (!req.body.username.match(/^[a-zA-Z0-9]+$/)) {
      return next(
        errorHander(400, 'Username can only contain letters and numbers')
      );
    }
  }
  try {
 ...
 export const  getUsers= async(req,res,next)=>{
  ...
  if(!req.user.isAdmin){
    return next(errorHander(403,"You are not allowed to see all users"))
  }
